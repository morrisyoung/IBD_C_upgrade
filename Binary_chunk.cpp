...
// special notes:
// we may possibly want to test IBD_hash; but by now it has now problems
...
void * chunk_level(void * p)
{
    ...
    //free(lca_env[i]);
    //lca_env.clear();
    lca_env_rep.clear();	// we'll still use this in the following session
    pool_env.clear();
