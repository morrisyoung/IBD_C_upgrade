// LCA_new.cpp
// function: I use the array to substitute some of the vectors; but there is no obvious improvement in the running speed
...
int E[30000];  // used as the list of labels
int E_pointer;

int L[30000];  // used as the list of levels
int L_pointer;

double r[30000];  // used for the internal nodes
int r_pointer;

int R[10000];  // used for the leaves

int list_up_val[5000];	// 5000: 714, so it won't go beyond 1000

int list_up_pos[5000];

vector<string> list_up_type;	// should be further initialized
...
void LCA_list_construction(char * tree)
{
	...
	while(tree[i] != ';')
	{
		...
		{
			i++;
			E[E_pointer++] = stack.back();
			L[L_pointer++] = stack.size()-1;
			continue;
		}
		...
		if(tree[i] >= 48 && tree[i] <= 57)
		{
			...
			E[E_pointer++] = node_value;
			L[L_pointer++] = stack.size();
			R[node_value-1] = E_pointer - 1;
